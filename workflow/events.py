...
class FinalAnswer(Event):
    ...
    """
    query:            str
    reasoning:        str               # 최종 답변 (근거 기반)
    cited_ids:        list[str]     # LLM이 답변에 사용한 근거 ID (검증 통과 부분집합)
    retrieved_ids:    list[str]     # 세 레인에서 검색된 전체 근거 ID (최대 8)
...
class SynthesisResponse(BaseModel):
    ...
    실재하는 항목만 cited_articles로 재구성한다(hallucination 차단).
    """
    reasoning:          str
